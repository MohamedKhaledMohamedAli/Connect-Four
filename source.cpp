...
bool winningMove(vector<vector<int> >&, unsigned int, int, int);
...
void playGame() {
	...
	while (!gameOver) { // while no game over state
		...
		gameOver = winningMove(board, currentPlayer,row ,col); // check if player won
...
array<int, 2> miniMax(vector<vector<int> > &b, unsigned int d, int alf, int bet, unsigned int p) {
	...
	if (p == COMPUTER) { // if AI player
		...
		if (winningMove(b, PLAYER,row,col)) { // if player about to win
		...
		if (winningMove(b, COMPUTER,row,col)) {
 ...
 * @param p - the player to check against
 * @param row - Row at which last move was played
 * @param col - col at which last move was played
 * @return - whether that player can have a winning move
 */
bool winningMove(vector<vector<int> > &b, unsigned int p , int row, int col) {
